...
object S2_C14_ByteAtATimeECBDecryptHarder {
  ...
  def run: Unit = {
    ...
    assert(S2_C9_PCKS7Padding.removePadding(byteAtATimeECBDecryptionHarder(oracle)).diff(oracle.secret).isEmpty)
  ...
  def findPrefixLength(oracle: ECBOracleHarder): Int = {
    ...
    val prefixLength = calcMultipleA(0).grouped(blockSize)
                                       ...
                                       .zipWithIndex
                                       .toArray
                                       .takeWhile(e => !e._1._1.diff(e._1._2).isEmpty)
                                       .map(_._2)
                                       .apply(0) * blockSize
    (0 until blockSize).find {i =>
      val blocks = calcMultipleA(2 * blockSize + i).grouped(blockSize).toArray
      blocks.zip(blocks.drop(1)).exists(v => v._1.diff(v._2).isEmpty)
